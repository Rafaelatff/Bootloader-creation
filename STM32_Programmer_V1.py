...
def process_COMMAND_BL_GET_CREV(length):
    value = read_serial_port(length)
    rev = (value[1] << 8 )+ value[0]
    print("\n   Chip Rev. : ",hex(rev))
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        ...
        print("\n   Command == > BL_GET_CREV")
        COMMAND_BL_GET_CREV_LEN             =6
        data_buf[0] = COMMAND_BL_GET_CREV_LEN-1 
        data_buf[1] = COMMAND_BL_GET_CREV 
        crc32       = get_crc(data_buf,COMMAND_BL_GET_CREV_LEN-4)
        crc32 = crc32 & 0xffffffff
        data_buf[2] = word_to_byte(crc32,1,1) 
        data_buf[3] = word_to_byte(crc32,2,1) 
        data_buf[4] = word_to_byte(crc32,3,1) 
        ...
        Write_to_serial_port(data_buf[0],1)
        for i in data_buf[1:COMMAND_BL_GET_CREV_LEN]:
            Write_to_serial_port(i,COMMAND_BL_GET_CREV_LEN-1)
...
def read_bootloader_reply(command_code):
    ...
    if(len(ack) ):
        ...
        if (a_array[0]== 0xA5):
            ...
            elif(command_code) == COMMAND_BL_GET_CREV:
                process_COMMAND_BL_GET_CREV(len_to_follow)
