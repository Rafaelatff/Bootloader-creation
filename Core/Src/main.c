...
/* USER CODE BEGIN PFP */
// void bootloader_uart_read_data(); //Adc em main.h
// void bootloader_jump_to_user_app(); //Adc em main.h
...
int main(void)
{
  ...
  while (1) {
	  //HAL_UART_Transmit(&huart2, (uint8_t*)somadata, (uint16_t)strlen(somadata), HAL_MAX_DELAY);
	  //HAL_UART_Transmit(&huart1, (uint8_t*)somadata, (uint16_t)strlen(somadata), HAL_MAX_DELAY);
...
void printmsg(char *format,...){
	...
	// Exrtact the argument list using VA APIs
	va_list args;
	va_start(args, format);
	vsprintf(str, format, args);
	HAL_UART_Transmit(D_UART, (uint8_t*)str, sizeof(str), HAL_MAX_DELAY);
	va_end(args);
#endif
}
	...
	while(1){
		...
		switch(bl_rx_buffer[1]){
            ...
            case BL_GET_CID:
                ...
                break;
            case BL_GET_CREV:
            	bootloader_handle_getcrev_cmd(bl_rx_buffer);
            	break;
...
void bootloader_handle_getcrev_cmd(uint8_t *pBuffer){
	...
	if (! bootloader_verify_crc(&bl_rx_buffer[0],command_packet_len-4,host_crc)) {
        printmsg("BL_DEBUG_MSG:checksum success !!\n");
        bootloader_send_ack(pBuffer[0],2);
        bl_crev_num = get_mcu_chip_rev();
        printmsg("BL_DEBUG_MSG:MCU id : %d %#x !!\n",bl_crev_num, bl_crev_num);
        bootloader_uart_write_data((uint8_t *)&bl_crev_num,2);
...
// Edit code according to memory size of the microcontroler
